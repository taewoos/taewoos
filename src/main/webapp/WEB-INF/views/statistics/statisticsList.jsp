...
<section class="main_container">
	...
	<!-- 검색 -->
	<form action="<c:url value='/statistics/statisticsList' />" class="navbar-form navbar-right search" role="search">
...
<div class="container">
	<table class="table table-bordered table-hover dt-responsive">
		...
		<tbody>
			<c:forEach var="vo" items="${statisticsList}">
				<tr>
					<td align="center">${vo.statisticsNo}</td>
					<td><a href="<c:url value='/statistics/statisticsDetail/${vo.statisticsNo}${pc.makeURI(pc.paging.pageNum)}' />">${vo.statisticsTitle}</a>
						&nbsp;
					...
					<td align="center">${vo.statisticsHit}</td>
					<td align="center"><fmt:formatDate value="${vo.statisticsDate}" pattern="yyyy-MM-dd" /></td>
				</tr>
			</c:forEach>			
		</tbody>
	</table>
	...
	<!-- 페이징 -->
	<form action="<c:url value='/statistics/statisticsList' />" name="pageForm">
		...
		<!-- 등록버튼 -->
		<a href="<c:url value='/statistics/statisticsRegist' />">
			<button type="button" class="btn btn-insert" onclick="location.href='<c:url value="/statistics/statisticsRegist" />'">게시글 등록</button>
		</a>
